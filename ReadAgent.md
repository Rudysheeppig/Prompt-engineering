ReadAgent是浙江大学、微软亚洲研究院合作开发的开源项目，以chatGPT和Hugging Face为基础构建的一个框架，融合LLM能力以及AI领域模型的能力，来解决不同领域和模态的AI任务。项目地址是：https://github.com/microsoft/JARVIS。 项目名称借鉴了漫威《钢铁侠》中人工智能管家的名称，也传递了项目想要实现通用人工智能的愿景。

ReadAgent 的新方法，这是一种受人类启发的大型语言模型 （LLM） 代理系统，旨在通过将有效上下文长度增加到 20 倍来处理非常长的上下文。受人类交互式阅读长文档的启发，ReadAgent 使用提示系统来决定在内存剧集中存储哪些内容，将这些剧集压缩成称为要点记忆的简短情节记忆，并在需要时采取措施查找原始文本中的段落。主要研究结果表明，ReadAgent在三个长文档阅读理解任务上的表现优于基线，同时显着扩展了有效上下文窗口。本文的主要贡献是开发了ReadAgent，并通过实验评估展示了其性能优势。

大型语言模型 （LLM） 在读取长输入方面存在局限性
ReadAgent 将有效上下文长度增加到 20×
受人类阅读方法的启发
ReadAgent 使用提示系统来决定内容存储，将剧集压缩到要点记忆中，并在需要时查找段落
在长文档阅读理解任务中表现优于基线
贡献包括 ReadAgent 的开发和通过评估证明的性能优势
